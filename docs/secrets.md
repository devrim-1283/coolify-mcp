...
- Run `npx @donedynamics/coolify-mcp doctor` on every machine that has ever had an MCP client
  on it.
...
`npx @donedynamics/coolify-mcp`, which is the primary install path. So this dispatches over the
platform's own CLI.
...
```bash
npx @donedynamics/coolify-mcp doctor                 # your coolify entry only
npx @donedynamics/coolify-mcp doctor --all-servers   # every MCP server in every client config
npx @donedynamics/coolify-mcp doctor --json          # machine-readable
npx @donedynamics/coolify-mcp doctor --fix           # conservative repair, see below
```
...
| `unpinned-version`           | info         | A client resolves `@donedynamics/coolify-mcp@latest` at spawn time.                                                                                                                             |
...
2. Put the replacement in an environment variable or a secret manager. Reference
   it; do not paste it.
3. Re-run `npx @donedynamics/coolify-mcp doctor` and confirm the finding is gone.
