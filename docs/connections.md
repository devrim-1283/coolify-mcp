...
```bash
npx @donedynamics/coolify-mcp connections          # every connection, its token source, whether it resolves
...
```bash
npx @donedynamics/coolify-mcp check                      # live probe, every connection
npx @donedynamics/coolify-mcp check --connection prod
```
